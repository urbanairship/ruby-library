...
module Urbanairship
  ...
  module ClassMethods
    attr_accessor :application_key, :application_secret, :master_secret,
                  :logger, :request_timeout, :provider
    ...
    def delete_scheduled_push(param)
      ...
      path = param.is_a?(Hash) ? "/api/push/scheduled/alias/#{param[:alias]}" : "/api/push/scheduled/#{param}"
    ...
    def verify_configuration_values(*symbols)
      ...
      fail("Must configure #{absent_values.join(', ')} before making this request.") unless absent_values.empty?
    end
    ...
    def parse_push_options(hash = {})
      hash[:aliases] = hash[:aliases].map(&:to_s) unless hash[:aliases].nil?
